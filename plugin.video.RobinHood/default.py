# -*- coding: utf-8 -*-
import urllib, urllib2, base64, re, os, xbmcplugin, xbmcgui, xbmcaddon, xbmcvfs, traceback, cookielib
xtr = 'VjFaV2IxVXdNVWhVYTFacFRURndUbFJVUW5aa01YQklUbFprVDJKVldscFdNalZoVkZkS1ZWWnVUbUZTVlRBeFZGVmFkMWRXVW5GUmF6Rk9UVWhCZVZZeFkzaGpNbEpZVW10b1VGZEdjR2hWYm5CSFl6RnNkRTVXV210V01VcEtXVlZTUWxCUlBUMD0='.decode('base64')
...
addon = xbmcaddon.Addon('plugin.video.RobinHood')
xrf_os = xtr.decode('base64')
...
awk_nk = rtm_os.decode('base64')
SOURCES = [{"title": "RobinHood pro list", "url": awk_nk.decode('base64'), "fanart": "https://dl.dropbox.com/s/30inzam8ovataac/mad_logo_by_yayomola-d511d44.jpg", "genre": "Live Tv", "date": "12.27.2016", "credits": "MadDog", "thumbnail": "https://dl.dropbox.com/s/dvyzrwuug26o1e3/madlogo-2.png"}]



def addon_log(string):
    if debug == 'true':
        xbmc.log("[addon.RobinHood-%s]: %s" %(addon_version, string))
...
def makeRequest(url, headers=None):
        try:
            ...
            if hasattr(e, 'code'):
                ...
                xbmc.executebuiltin("XBMC.Notification(RobinHood,We failed with error code - "+str(e.code)+",10000,"+icon+")")
            elif hasattr(e, 'reason'):
                addon_log('We failed to reach a server.')
                addon_log('Reason: %s' %e.reason)
                xbmc.executebuiltin("XBMC.Notification(RobinHood,We failed to reach a server. - "+str(e.reason)+",10000,"+icon+")")
...
def addSource(url=None):
        ...
        xbmc.executebuiltin("XBMC.Notification(RobinHood,New source added.,5000,"+icon+")")
        if not url is None:
...
def urlsolver(url):
    ...
    if host:
        ...
        xbmc.executebuiltin("XBMC.Notification(RobinHood,Urlresolver donot support this domain. - ,5000)")
        resolver=url
    return resolver
...
def download_file(name, url):
        if addon.getSetting('save_location') == "":
            xbmc.executebuiltin("XBMC.Notification('RobinHood','Choose a location to save files.',15000,"+icon+")")
            addon.openSettings()
        params = {'url': url, 'download_path': addon.getSetting('save_location')}
        downloader.download(name, params)
        dialog = xbmcgui.Dialog()
        ret = dialog.yesno('RobinHood', 'Do you want to add this file as a source?')
        if ret:
...
#            print 'parentalblockedpin',parentalblockedpin
            ...
            if showcontext == 'source':
                ...
                contextMenu.append(('Remove from RobinHood Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
            ...
            if not name in FAV:
                contextMenu.append(('Add to RobinHood Favorites','XBMC.RunPlugin(%s?mode=5&name=%s&url=%s&iconimage=%s&fanart=%s&fav_mode=%s)'
...
#                print 'setting as folder in link'
        ...
        if showcontext:
            ...
            if showcontext == 'fav':
                contextMenu.append(
                    ('Remove from RobinHood Favorites','XBMC.RunPlugin(%s?mode=6&name=%s)'
                     %(sys.argv[0], urllib.quote_plus(name)))
                     )
                ...
                contextMenu.append(('Add to RobinHood Favorites','XBMC.RunPlugin(%s)' %fav_params))
            liz.addContextMenuItems(contextMenu)
...
#            ln+='</item>'
        ...
        if url:
            ...
            xbmc.executebuiltin("XBMC.Notification(RobinHood,Failed to extract regex. - "+"this"+",4000,"+icon+")")
    ...
    try:
        ...
        xbmc.executebuiltin("XBMC.Notification(RobinHood,Please [COLOR yellow]install Youtube-dl[/COLOR] module ,10000,"")")
    ...
    if not (keyboard.isConfirmed() == False):
        ...
        if newStr==parentalblockedpin:
            ...
            xbmc.executebuiltin("XBMC.Notification(RobinHood,Parental Block Disabled,5000,"+icon+")")
        else:
            xbmc.executebuiltin("XBMC.Notification(RobinHood,Wrong Pin??,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))

elif mode==56:
    addon_log("disable lock")
    addon.setSetting('parentalblocked', "true")
    xbmc.executebuiltin("XBMC.Notification(RobinHood,Parental block enabled,5000,"+icon+")")
    xbmcplugin.endOfDirectory(int(sys.argv[1]))
